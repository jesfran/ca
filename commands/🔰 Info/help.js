  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      if (args[0]) {
        ...
        let button_back = new MessageButton().setStyle('PRIMARY').setCustomId('1').setEmoji("933455622878396468").setLabel(handlemsg(client.la[ls].cmds.info.help.buttons.back))
        ...
        let button_forward = new MessageButton().setStyle('PRIMARY').setCustomId('3').setEmoji('933455679312777316').setLabel(handlemsg(client.la[ls].cmds.info.help.buttons.forward))        
        let button_tutorial = new MessageButton().setStyle('LINK').setEmoji("930608434603262045").setLabel("Support").setURL("https://discord.gg/Azury")
        let menuOptions = [
          {
            label: "Overview",
            value: "Overview",
        ...
        .setTitle(`<:hydroxcategory:936682160830681178> Information about __${client.user.username}__`)
        .addField("<:yes:936697617763663913> **__Mis novedades__**",
`>>> Tengo más de **58+ sistemas y más de 500 comandos y funciones.** Esto incluye un **sistema avanzado de solicitud de música <:Spotify:933246700003409920>**, Sistema de rol de reacción, 🕹️ Más de **150+** Diversión y**🎮 Comandos de minijuegos**. Incluso incluye un  **Welcomer & Leaver** con el sistema de seguimiento de invitaciones! ¡Una de las mejores partes es su sistema de 🚫 **Administración** y **Auto-Moderación**! Hay muchas más cosas, ${client.user.username} ¡Es el mejor bot tu ** tu ** servidor!`)
        .addField(":question: **__¿Cómo me usas?__**",
`>>> mi prefijo es \`${prefix}\`, usa \`${prefix}help\` para ver todos los comandos\n**Para configurar un uso de **sistema**, \`${prefix}setup\` o \`${prefix}setup-SYSTEM\`!**`)
.addField(":chart_with_upwards_trend: **__Estadisticas:__**",
...
<a:Valid_Code_Developer:933742305398116363> Hecho por **[jesfran 💫](https://discord.gg/BVTuGNFfXh)**`)
.addField(":question: ¿Cómo obtener ayuda?", `>>> **\`1. vía \`** *Usa los botones para cambiar las páginas*\n**\`2 vía \`** *Use el menú para seleccionar todas las páginas de ayuda que desea mostrar*\n**\`3 vía \`** *Contactame en <@!326964325346443266>*`)

       
        //Send message with buttons
        let helpmsg = await message.reply({   
            content: `***Tocame para cambiar la pestaña de ayuda***`,
        ...
        const alldisabledbuttons = [d_buttonRow, d_dropRow]
          if(!edited){
            edited = true;
        ...
        function allotherembeds_eachcategory(filterdisabled = false){
          ...
          var embed0 = new MessageEmbed()
            ...
            .addField(`<:team_members:938158293434630145> **User Commands**`, ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "user").sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            ...
            .addField(`<:Discord:933396674502594603> **Server Related Commands**`,  ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "server").sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField(`<:Bot_Flag:933409742796259339> **Bot Related Commands**`,  ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "bot").sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField(`<:Builder:933410104978599936> **Util Related Commands**`, ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "util").sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
          embeds.push(embed0)

          //ECONOMY COMMANDS
          var embed1 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "💸 Economy").size}\`] 💸 Economy Commands 💸 | ${settings.ECONOMY ? "<a:yes:933036759913201725> ENABLED" : "<:emojino:933030632731340872> DISABLED"}`)
            ...
            .addField(`<:Builder:933410104978599936> **Information & Manage 💸**`,  ">>> " + client.commands.filter((cmd) => cmd.category === "💸 Economy" && cmd.type === "info").sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            if(!filterdisabled || settings.ECONOMY || settings.showdisabled) embeds.push(embed1)

          //SCHOOL COMMANDS
          var embed2 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "🏫 School Commands").size}\`] 🏫 School Commands 🏫 | ${settings.SCHOOL ? "<a:yes:933036759913201725> ENABLED" : "<:emojino:933030632731340872> DISABLED"}`)
          ...
          var embed3 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "🎶 Music").size}\`] 🎶 Music Commands 🎶 | ${settings.MUSIC ? "<a:yes:933036759913201725> ENABLED" : "<:emojino:933030632731340872> DISABLED"}`)
            ...
            .addField("<a:playing:933421997566853140> **Song Commands**", "> "+client.commands.filter((cmd) => cmd.category === "🎶 Music" && cmd.type.includes("song")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField("<:Bot_Flag:933409742796259339> **Bot Commands**", "> "+client.commands.filter((cmd) => cmd.category === "🎶 Music" && cmd.type.includes("bot")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            if(!filterdisabled || settings.MUSIC || settings.showdisabled) embeds.push(embed3)

          //FILTER COMMANDS
          var embed4 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "👀 Filter").size}\`] 👀 Filter Commands 👀 | ${settings.FILTER ? "<a:yes:933036759913201725> ENABLED" : "<:emojino:933030632731340872> DISABLED"}`)
          ...
          var embed5 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "⚜️ Custom Queue(s)").first().extracustomdesc.length}\`] ⚜️ Custom Queue(s) Commands ⚜️ | ${settings.CUSTOMQUEUE ? "<a:yes:933036759913201725> ENABLED" : "<:emojino:933030632731340872> DISABLED"}`)
            .setDescription(`> *${client.commands.filter((cmd) => cmd.category === "⚜️ Custom Queue(s)").first().extracustomdesc.split(",").map(i => i?.trim()).join("︲")}*`)
            .addField("\u200b", "\u200b")
            .addField("<a:yes:933036759913201725>  **Usage**", "> "+client.commands.filter((cmd) => cmd.category === "⚜️ Custom Queue(s)").first().usage)
          ...
          var embed6 = new MessageEmbed()
            ...
            .addField("\u200b", "__**Sub-Categorized Commands:**__")
            .addField("<:Discord:933396674502594603> **Server Related Commands**", "> "+client.commands.filter((cmd) => cmd.category === "🚫 Administration" && cmd.type.includes("server")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField("<:Channel:933422338618327080> **Channel Related Commands**", "> "+client.commands.filter((cmd) => cmd.category === "🚫 Administration" && cmd.type.includes("channel")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField("<:ThreadChannel:933422481719562310> **Thread Related Commands**", "> "+client.commands.filter((cmd) => cmd.category === "🚫 Administration" && cmd.type.includes("thread")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField("<:Roles:933422596421197875> **Role Related Commands**", "> "+client.commands.filter((cmd) => cmd.category === "🚫 Administration" && cmd.type.includes("role")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField("<:rmembers:936684824410873867> **Member Related Commands**", "> "+client.commands.filter((cmd) => cmd.category === "🚫 Administration" && cmd.type.includes("member")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
          embeds.push(embed6)

          //SETUP
          var embed7 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "💪 Setup").size}\`] <a:disbotlist:936684977612017691> Setup Commands <a:disbotlist:936684977612017691>`)
            ...
            .addField("<:Builder:933410104978599936> **Security Systems**", "> "+client.commands.filter((cmd) => cmd.category === "💪 Setup" && cmd.type.includes("security")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
          embeds.push(embed7)
          ...
          var embed8 = new MessageEmbed()
            ...
            .addField("<:Bot_Flag:933409742796259339> **Bot Related Commands**", "> "+client.commands.filter((cmd) => cmd.category === "⚙️ Settings" && cmd.type.includes("bot")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
          ...
          var embed9 = new MessageEmbed()
            ...
            .addField("\u200b", "__**Sub-Categorized Commands:**__")
            .addField("<:Discord:933396674502594603> **Information & Manage**", "> "+client.commands.filter((cmd) => cmd.category === "👑 Owner" && cmd.type.includes("info")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField("<:Bot_Flag:933409742796259339> **Adjust the Bot**", "> "+client.commands.filter((cmd) => cmd.category === "👑 Owner" && cmd.type.includes("bot")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            embeds.push(embed9)
          
          //Programming Commands
          var embed10 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "⌨️ Programming").size}\`] ⌨️ Programming Commands ⌨️ | ${settings.PROGRAMMING ? "<a:yes:933036759913201725> ENABLED" : "<:emojino:933030632731340872> DISABLED"}`)
          ...
          var embed11 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "📈 Ranking").size}\`] 📈 Ranking Commands 📈 | ${settings.RANKING ? "<a:yes:933036759913201725> ENABLED" : "<:emojino:933030632731340872> DISABLED"}`)
            ...
            .addField("\u200b", "__**Sub-Categorized Commands:**__")
            .addField("<:Builder:933410104978599936> **Manage Rank**", `> ${client.commands.filter((cmd) => cmd.category === "📈 Ranking" && cmd.type === "manage").sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲")}`)
          ...
          var embed12 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "🔊 Soundboard").size}\`] 🔊 Soundboard Commands 🔊 | ${settings.SOUNDBOARD ? "<a:yes:933036759913201725> ENABLED" : "<:emojino:933030632731340872> DISABLED"}`)
          ...
          var embed13 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "🎤 Voice").first().extracustomdesc.length}\`] 🎤 Voice Commands 🎤 | ${settings.VOICE ? "<a:yes:933036759913201725> ENABLED" : "<:emojino:933030632731340872> DISABLED"}`)
            .setDescription(`> *${client.commands.filter((cmd) => cmd.category === "🎤 Voice").first().extracustomdesc.split(",").map(i => i?.trim()).join("︲")}*`)
            .addField("\u200b", "\u200b")
            .addField("<a:yes:933036759913201725>  **Usage**", "> "+client.commands.filter((cmd) => cmd.category === "🎤 Voice").first().usage)
          if(!filterdisabled || settings.VOICE || settings.showdisabled) embeds.push(embed13)
          
          //FUN COMMANDS
          var embed14 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "🕹️ Fun").size}\`] 🕹️ Fun Commands 🕹️ | ${settings.FUN ? "<a:yes:933036759913201725> ENABLED" : "<:emojino:933030632731340872> DISABLED"}`)
          ...
          var embed15 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "🎮 MiniGames").size}\`] 🎮 Mini Games Commands 🎮 | ${settings.MINIGAMES ? "<a:yes:933036759913201725> ENABLED" : "<:emojino:933030632731340872> DISABLED"}`)
            .addField("\u200b", "__**Sub-Categorized Commands:**__")
          ...
          var embed16 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "😳 Anime-Emotions").size}\`] 😳 Anime Commands 😳 | ${settings.ANIME ? "<a:yes:933036759913201725> ENABLED" : "<:emojino:933030632731340872> DISABLED"}`)
          ...
          var embed17 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "🔞 NSFW").size}\`] 🔞 NSFW Commands 🔞 | ${settings.NSFW ? "<a:yes:933036759913201725> ENABLED" : "<:emojino:933030632731340872> DISABLED"}`)
