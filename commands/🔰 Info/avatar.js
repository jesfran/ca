    ...
    try {
      ...
      let embed = new MessageEmbed()
        .setAuthor(handlemsg(client.la[ls].cmds.info.avatar.author, {
          ...
          dynamic: true
        }), "https://discord.gg/BVTuGNFfXh")
        .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
        .addField("<:arrow:933397939114623096> PNG", `[\`LINK\`](${user.displayAvatarURL({format: "png"})})`, true)
        .addField("<:arrow:933397939114623096> JPEG", `[\`LINK\`](${user.displayAvatarURL({format: "jpg"})})`, true)
        .addField("<:arrow:933397939114623096> WEBP", `[\`LINK\`](${user.displayAvatarURL({format: "webp"})})`, true)
