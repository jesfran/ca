  ...
  client.on("guildMemberAdd", async mem => {
    ...
    async function welcome(member) {
      ...
      if (invite && inviter) {
        ...
        invitemessage = `Invitado por ${inviter.tag ? `**${inviter.tag}**` : `<@${inviter.id}>`}\n<:Like:933407545693990933> **${realinvites} Invitación${realinvites == 1 ? "" : "es"}**\n[<:joines:933407723834474599> ${invites} Joins | <:bye:933461028593885234> ${leaves} Leaves | <:emojino:933030632731340872> ${fake} Fakes]`;
      } 
      else if(invite){
      ...
      if (client.settings.get(member.guild.id, "welcome.captcha") && !member.user.bot) {
       const captcha = new Captcha();
        ...
        member.send({content: `**${member.guild.name}** tiene una opción de seguridad Captcha habilitada!\n> ¡Resuélvalo primero, escribiendo las LETRAS COMPLETAS de la IMAGEN!`, embeds: [captchaembed], files: [attachment]}).then(msg => {
