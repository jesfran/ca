...
module.exports = (client) => {
  ...
  client.on("messageCreate", async (message) => {
    ...
    client.settings.ensure(message.guild.id, {
        suggest: {
          ...
          footertext: `¿Quieres sugerir/comentarios sobre algo? ¡Simplemente escribe en este canal!`,
    ...
    let whobutton = new MessageButton().setStyle("PRIMARY").setEmoji("❓").setCustomId("Suggest_who").setLabel("¿Quién votó?")
    ...
    if (message.channel.id === feedbackchannel) {
      ...
      var embed = new MessageEmbed()
        ...
        .addField(`:thumbsup: **__Votos arriba__**`, `**\`\`\`0 Votos\`\`\`**`, true)
        .addField(`:thumbsdown: **__Votos negativos__**`, `**\`\`\`0 Votos\`\`\`**`, true)
        .setColor(es.color)
        .setAuthor(client.getAuthor(message.author.tag + "' Sugerencia", message.member.user.displayAvatarURL({ dynamic: true }), `https://discord.com/api/oauth2/authorize?client_id=${client.user.id}&permissions=8&scope=bot`))
        .setDescription("\n" + message.content + "\n")
  ...
  client.on("interactionCreate", async (button) => {
      ...
      if (button?.customId.startsWith("Suggest_")) {
          ...
          if(button?.customId == "Suggest_who"){
            return button?.reply({
              ...
              embeds: [
                ...
                .addField(`${SuggestionsData.upvotes} votos a favor`,`${SuggestionsData.voted_ppl && SuggestionsData.voted_ppl.length > 0 ? SuggestionsData.voted_ppl.length < 20 ? SuggestionsData.voted_ppl.map(r => `<@${r}>`).join("\n") : [...SuggestionsData.voted_ppl.slice(0, 20).map(r => `<@${r}>`), `${SuggestionsData.voted_ppl.length - 20} more...`].join("\n") : "Nadie"}`.substring(0, 1024), true)
          ...
          embed.fields[0].key = `:thumbsup: **__Votos arriba__**`;
