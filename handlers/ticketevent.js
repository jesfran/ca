        ...
        if(interaction?.guildId && interaction?.isSelectMenu() && interaction?.message && interaction?.message.author.id == client.user.id){
            ...
            if(message.id == settings.messageId && (channelId == settings.channelId || message.channelId == settings.channelId)){
               let index = settings.data.findIndex(v => v.value == values[0]);
                ...
                let optionsData = {
                topic: `📨 Ticket para: ${user.tag} (${user.id}) | ${values[0]} | ✅ Creado en: ${moment().format("LLLL")}`,
                    type: "GUILD_TEXT",
                ...
                let button_delete = new MessageButton().setStyle('SECONDARY').setCustomId('ticket_delete').setLabel("Borrar").setEmoji("941906625432793220")
                ...
                if(settings.claim.enabled){
                    allbuttons.push(new MessageActionRow().addComponents([new MessageButton().setStyle('SECONDARY').setCustomId('ticket_claim').setLabel("Reclama el Boleto").setEmoji("933036759913201725")]))
                }
