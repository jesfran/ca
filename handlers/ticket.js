...
module.exports = (client, preindex) => {
  ...
  client.on("interactionCreate", async (interaction) => {
    ...
    client.setups.ensure(guild.id, {
      ...
      claim: {
        enabled: false,
        messageOpen: "Querido {user}!\n> *¡Espere hasta que un miembro del personal reclame su boleto!*",
                messageClaim: "{claimer} **ha reclamado el billete!**\n> Él ahora dará {user} apoyo!"
              },
              message: "Hey {user}, gracias por abrir un ticket! Alguien te ayudará pronto!",
              adminroles: []
    }, systempath);
    ...
    let optionsData = {
      topic: `📨 #${String(filename).replace("ticket", "").length > 0 ? String(filename).replace("ticket", "") : "1"} boleto para: ${user.tag} (${user.id}) | ✅ Creado en: ${moment().format("LLLL")}`,
      type: "GUILD_TEXT",
    ...
    guild.channels.create(channelname.substring(0, 31), optionsData).then(async ch => {
      ...
      var ticketembed = new MessageEmbed()
        ...
        .setFooter(client.getFooter(`Para cerrar/gestionar este ticket reacciona con los botones\nTambién puede escribir: ${client.settings.get(guild.id, "prefix")}ticket`, es.footericon))
        .setAuthor(client.getAuthor(`boleto para: ${user.tag}`, user.displayAvatarURL({
          dynamic: true
        }), "https://discord.gg/cxAdRfwz"))
      ...
      if (ticket.claim.enabled) {
        var claimEmbed = new MessageEmbed()
          ...
          .setAuthor(client.getAuthor(`Un miembro del personal reclamará el boleto pronto!`, "https://cdn.discordapp.com/emojis/833101350623117342.gif?size=44", "https://discord.gg/milrato"))
      ...
      let button_close = new MessageButton().setStyle('PRIMARY').setCustomId('ticket_close').setLabel('Cerrar').setEmoji("941906330334146612")
      let button_delete = new MessageButton().setStyle('SECONDARY').setCustomId('ticket_delete').setLabel("Eliminar").setEmoji("941906625432793220")
      let button_transcript = new MessageButton().setStyle('PRIMARY').setCustomId('ticket_transcript').setLabel("Transcripción").setEmoji("📑")
      let button_user = new MessageButton().setStyle('SUCCESS').setCustomId('ticket_user').setLabel("Usuarios").setEmoji("941906962214432798")
      let button_role = new MessageButton().setStyle('SUCCESS').setCustomId('ticket_role').setLabel("Roles").setEmoji("941907220998783006")
      const allbuttons = [new MessageActionRow().addComponents([button_close, button_delete, button_transcript, button_user, button_role])]
      if (ticket.claim.enabled) {
        allbuttons.push(new MessageActionRow().addComponents([new MessageButton().setStyle('SECONDARY').setCustomId('ticket_claim').setLabel("Reclama el Boleto").setEmoji("941828874575175751")]))
      }
