...
module.exports = client => {
        ...
        switch(interaction?.customId){
            ...
            case "Skip": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`⏭ **¡Pasó a la siguiente canción!**`)
                  .setFooter(client.getFooter(`💢 Acción por: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
            ...
            case "Autoplay": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.get(`autoplay`) ? `<a:yes:933036759913201725> **Reproducción automática habilitada**`: `<:emojino:933030632731340872> **Reproducción automática deshabilitada**`}`)
                  .setFooter(client.getFooter(`💢 Acción por: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
            ...
            case "Song": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.trackRepeat ? `<a:yes:933036759913201725> **Bucle de canción habilitado**`: `<:emojino:933030632731340872> **Bucle de canción deshabilitado**`}`)
                  .setFooter(client.getFooter(`💢 Acción por: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
            ...
            case "Forward": {
                ...
                interaction?.reply({
                  embeds: [new MessageEmbed()
                    .setColor(ee.color)
                    .setTimestamp()
                    .setTitle(`⏩ **Adelanta la canción para \`10 Segundos\`!**`)
                    .setFooter(client.getFooter(`💢 Acción por: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
            ...
            case "Rewind": {
                ...
                interaction?.reply({
                  embeds: [new MessageEmbed()
                    .setColor(ee.color)
                    .setTimestamp()
                    .setTitle(`⏪ **Regreso la canción durante \ '10 segundos\`!**`)
                    .setFooter(client.getFooter(`💢 Acción por: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
    ...
    client.on("messageCreate", async message => {
        ...
        if (!channel) return message.reply("<:emojino:933030632731340872> **¡Únete primero a un canal de voz!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 5000)})
        ...
        if (player && channel.id !== player.voiceChannel) return message.reply(`<:emojino:933030632731340872> **Únase primero a __my__ Voice Channel! <#${player.voiceChannel}>**`).then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
...
function generateQueueEmbed(client, guildId, leave){
    ...
    if(!leave && player && player.queue && player.queue.current){
        ...
        .setColor(es.color)
        .setDescription(String(songs.map((track, index) => `**\` ${++index}. \` ${track.uri ? `[${track.title.substr(0, 60).replace(/\[/igu, "\\[").replace(/\]/igu, "\\]")}](${track.uri})` : track.title}** - \`${track.isStream ? `TRANSMISIÓN EN VIVO` : format(track.duration).split(` | `)[0]}\`\n> *Solicitado por: __${track.requester.tag}__*`).join(`\n`)).substr(0, 2048));
    ...
    var autoplaybutton = new MessageButton().setStyle('SUCCESS').setCustomId('Autoplay').setEmoji('937783971985645708').setLabel(`Auto-reproducción`).setDisabled();
    var songbutton = new MessageButton().setStyle('SUCCESS').setCustomId('Song').setEmoji(`937783971985645708`).setLabel(`Canción`).setDisabled();
